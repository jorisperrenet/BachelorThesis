\chapter{Proofs concerning Cotesian numbers}
\label{app:proofs}
...
\begin{equation}
    c_i =
        % \frac{1}{(n-1)!} \sum_{j=0}^{n} \frac{n^j (-1)^{n} s(n+1, j+1) }{j+1} & i = 0 \\
        \frac{1}{(n-1)!} \binom{n}{i} \sum_{j=0}^{n} \sum_{m=0}^{n-j} i^m n^{j} \frac{(-1)^{i+n} s(n+1, j+m+1) }{j+1} \,. \nonumber
        % \frac{1}{(n-1)!} \sum_{j=0}^{n} \frac{n^{j} s(n,j)}{j+1} & i = n
\end{equation}
\end{theorem}
...
\begin{align}
    c_i &= \sum_{j=0}^{n} n^{j+1} \frac{(-1)^{n-j} e_{n-j} (\{ 1, \dots, n \} \setminusD \{ i \})}{(j+1) i! (n-i)! (-1)^{n-i}} \nonumber \\
        ...
        &= \frac{1}{(n-1)!} \binom{n}{i} \sum_{j=0}^{n} \sum_{m=0}^{n-j} i^m n^{j} \frac{(-1)^{i+n} s(n+1, j+m+1) }{j+1} \,. \nonumber
\end{align}
% We see that for $i=0$ (which we separately derive from equation \ref{eq:ci}), using Lemma \ref{lemma:complete}, we obtain the relation
% \begin{align}
%     c_0 &= \sum_{j=0}^{n} n^{j+1} \frac{(-1)^{n-j} e_{n-j} (\{ 1, \dots, n \} \setminusD \{ 0 \})}{(j+1) \prod^{n}_{m'=0, m'\neq 0} (0 - m')} \nonumber \\
%         &= \frac{1}{(n-1)!} \sum_{j=0}^{n} \frac{n^j (-1)^{n} s(n+1, j+1) }{j+1} \,. \nonumber
%         % &= \frac{1}{(n-1)!} \sum_{j=0}^{n} \frac{n^j (-1)^{n} [s(n,j)] }{j+1} + \frac{1}{(n-1)!} \sum_{j=0}^{n-1} \frac{n^j (-1)^{n} [-ns(n,j+1)] }{j+1} \nonumber \\
%         % &= \frac{1}{(n-1)!} \sum_{j=0}^{n} \frac{n^j (-1)^{n} [s(n,j)] }{j+1} + \frac{1}{(n-1)!} \sum_{j=1}^{n} \frac{n^{j} (-1)^{n+1} [s(n,j)] }{j} \nonumber \\
%         % &= \left[ \frac{1}{(n-1)!} \frac{n^j (-1)^{n} [s(n,j)] }{j+1} \right]_{j=0} + \frac{(-1)^n}{(n-1)!} \sum_{j=1}^{n} \frac{n^j s(n,j) }{j+1} - \frac{n^{j} s(n,j) }{j} \nonumber \\
%         % &= \left[ \frac{1}{(n-1)!} \frac{n^j (-1)^{n} [s(n,j)] }{j+1} \right]_{j=0} + \frac{(-1)^n}{(n-1)!} \sum_{j=1}^{n} \frac{j n^j s(n,j) - (j+1) n^{j} s(n,j) }{j(j+1)} \nonumber \\
%         % &= \left[ \frac{1}{(n-1)!} \frac{n^j (-1)^{n} [s(n,j)] }{j+1} \right]_{j=0} + \frac{(-1)^n}{(n-1)!} \sum_{j=1}^{n} \frac{-n^j s(n,j)}{j(j+1)} \nonumber
% \end{align}
% Likewise, for the case $i=n$ equation \ref{eq:ci} can be simplified using Lemma \ref{lemma:complete}, resulting in
% \begin{align}
%     c_n &= \sum_{j=0}^{n} n^{j+1} \frac{(-1)^{n-j} e_{n-j} (\{ 1, \dots, n \} \setminusD \{ n \})}{(j+1) n! (n-n)! (-1)^{n-n}} \nonumber \\
%         &= \frac{1}{(n-1)!} \sum_{j=0}^{n} \frac{n^{j} (-1)^{n-j} e_{n-j} (\{ 1, \dots, n-1 \})}{j+1} \nonumber \\
%         &= \frac{1}{(n-1)!} \sum_{j=0}^{n} \frac{n^{j} s(n,j)}{j+1} \,. \nonumber
% \end{align}
Proving the theorem.
\end{proof}
...
% \begin{proof}
% Some cases \textsc{WE MUST HAVE $n > 0$ FOR THE LAST STEP}:
% \begin{align}
%     c_n &= \sum_{j=0}^{n} n^{j+1} \frac{(-1)^{n-j} e_{n-j} (\{ 0, \dots, n \} \setminus \{ n \})}{(j+1) \prod^{n}_{m'=0, m'\neq n} (n - m')} \nonumber \\
%         &= \sum_{j=0}^{n} n^{j+1} \frac{(-1)^{n-j} e_{n-j} (\{ 0, \dots, n-1 \})}{(j+1) \prod^{n-1}_{m'=0} (n - m')} \nonumber \\
%         &= \frac{1}{(n-1)!} \sum_{j=0}^{n} n^{j} \frac{(-1)^{n-j} e_{n-j} (\{ 0, \dots, n-1 \})}{(j+1)} \nonumber \\
%         &= \frac{1}{(n-1)!} \sum_{j=0}^{n} n^{j} \frac{(-1)^{n-j} e_{n-j} (\{ 1, \dots, n-1 \})}{(j+1)} \nonumber \\
%         &= \frac{1}{(n-1)!} \sum_{j=0}^{n} \frac{n^j s(n,j)}{(j+1)} \nonumber \\
%         &= \frac{1}{(n-1)!} \sum_{j=1}^{n} \frac{n^j s(n,j)}{(j+1)} \nonumber
% \end{align}
% Note, that from lemma 2:
% \begin{equation}
%     e_j(\{ 1, \dots, \ell \}) = |s(\ell+1, \ell+1-j)| \nonumber
% \end{equation}
% Hence:
% \begin{equation}
%     e_{n-j}(\{ 1, \dots, n \}) = |s(n+1, n+1-(n-j))| = |s(n+1, j+1)| = (-1)^{n-j} s(n+1, j+1) \nonumber
% \end{equation}
% Thus:
% \begin{align}
%     c_0 &= \sum_{j=0}^{n} n^{j+1} \frac{(-1)^{n-j} e_{n-j} (\{ 0, \dots, n \} \setminus \{ 0 \})}{(j+1) \prod^{n}_{m'=0, m'\neq 0} (0 - m')} \nonumber \\
%         &= \sum_{j=0}^{n} n^{j+1} \frac{(-1)^{n-j} e_{n-j} (\{ 1, \dots, n \} )}{(j+1) (-1)^n n!} \nonumber \\
%         &= \frac{1}{(n-1)!} \sum_{j=0}^{n} \frac{n^j (-1)^{j} e_{n-j} (\{ 1, \dots, n \})}{(j+1)} \nonumber \\
%         &= \frac{1}{(n-1)!} \sum_{j=0}^{n} \frac{n^j (-1)^{j} (-1)^{n-j} s(n+1, j+1)}{(j+1)} \nonumber \\
%         &= \frac{(-1)^{n}}{(n-1)!} \sum_{j=0}^{n} \frac{n^j s(n+1, j+1)}{j+1} \nonumber
% \end{align}
% Else:
% \begin{equation}
%     c_i = \sum_{j=0}^{n} n^{j+1} \frac{(-1)^{n-j} e_{n-j} (\{ 0, \dots, n \} \setminus \{ i \})}{(j+1) \prod^{n}_{m'=0, m'\neq i} (i - m')} \nonumber
% \end{equation}
% Thus:
% \begin{align}
%     c_i &= \sum_{j=0}^{n} n^{j+1} \frac{(-1)^{n-j} e_{n-j} (\{ 0, \dots, n \} \setminus \{ i \})}{(j+1) i! (n-i)! (-1)^{n-i}} \nonumber \\
%         &= \frac{1}{n!} \binom{n}{i} \sum_{j=0}^{n} n^{j+1} \frac{(-1)^{j} e_{n-j} (\{ 0, \dots, n \} \setminus \{ i \})}{(j+1) (-1)^{i}} \nonumber \\
%         &= \frac{1}{(n-1)!} \binom{n}{i} \sum_{j=0}^{n} n^j \frac{(-1)^{j-i} e_{n-j} (\{ 0, \dots, n \} \setminus \{ i \})}{(j+1)} \nonumber \\
%         &= \frac{1}{(n-1)!} \binom{n}{i} \sum_{j=0}^{n} n^j \frac{(-1)^{j-i}}{(j+1)} \sum_{k=0}^{n-j} e_{k} (\{ 0, \dots, i-1 \}) e_{n-j-k} (\{ i+1, \dots, n \}) \nonumber \\
%         &= \frac{1}{(n-1)!} \binom{n}{i} \sum_{j=0}^{n} n^j \frac{(-1)^{j-i}}{(j+1)} \sum_{k=0}^{n-j} (-1)^k s(i,i-k) e_{n-j-k} (\{ i+1, \dots, n \}) \nonumber \\
%         &= \frac{1}{(n-1)!} \binom{n}{i} \sum_{j=0}^{n} \sum_{k=0}^{n-j} n^j \frac{(-1)^{j-i}}{(j+1)} (-1)^k s(i,i-k) e_{n-j-k} (\{ i+1, \dots, n \}) \nonumber
% \end{align}
% Also, we have:
% \begin{equation}
%     e_{k} (\{ i+1, \dots, n \}) = e_{k} (\{ i, \dots, n \}) - i e_{k-1} (\{ i+1, \dots, n \}) \nonumber
% \end{equation}
% ANd:
% \begin{equation}
%     e_{k} (\{ 1, \dots, n \}) = (-1)^k s(n+1, n+1-k) \nonumber
% \end{equation}
% Renaming:
% \begin{equation}
%     d (k, i) = d(k, i-1) - (i-1) d (k-1, i) \nonumber
% \end{equation}
% ANd:
% \begin{equation}
%     d (k, 1) = (-1)^k s(n+1, n+1-k) \nonumber
% \end{equation}
% \begin{equation}
%     d (0, i) = 1 \nonumber
% \end{equation}
...
% \begin{proof}
% We proof this now, rewriting the equation gives
% \begin{equation}
%     \begin{bmatrix}
%         c_0 \\
%         c_1 \\
...
% \textsc{THIS PROOF HAS NOT BEEN COMPLETED YET}
% \end{proof}
